...
module.exports = {
    ...
    merge: function (depsToMerge) {
        var deps = [].concat(depsToMerge).map(function (item) {
            return _convertFromMod(item);
        }.bind(this));
        return _convertToMod(decl.merge.apply(this, deps));
    },
    ...
    subtract: function (from, what) {
        return _convertToMod(decl.subtract(_convertFromMod(from) ,_convertFromMod(what)));
    },
    ...
    intersect: function (depsToIntersect) {
        ...
        return _convertToMod(decl.intersect.apply(this, deps));
    },
