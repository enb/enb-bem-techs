...
module.exports = inherit(/** @lends DepsResolver.prototype */{
                        ...
                        if (!dep.tech) {
                            if (dep.mustDeps) {
                                _this.normalizeDeps(dep.mustDeps, decl.name, decl.elem).forEach(function (nd) {
                                    ...
                                    console.log(key, {mustDeps: mustDeps.map(v => v.key), shouldDeps: shouldDeps.map(v => v.key)});
                                });
                            }
                            if (dep.shouldDeps) {
    ...
    addDecl: function (decl) {
        ...
        return this.getDeps(decl).then(function (deps) {
            ...
            // naturalization
            if (decl.elem && !decl.deps[decl.name]) {
                decl.deps[decl.name] = 'fake';
                decl.depCount++;
            }
            if (decl.modName) {
                if (decl.elem) {
                    if (!decl.deps[decl.name + '_' + decl.elem]) {
                        decl.deps[decl.name + '_' + decl.elem] = 'fake';
                        decl.depCount++;
                    }
                } else if (!decl.deps[decl.name]) {
                    decl.deps[decl.name] = 'fake';
                    decl.depCount++;
                }
            }
            ...
            return _this.addDecls(deps.mustDeps, function (dep) {
                ...
                console.log(key + ' => ' + dep.key, dep.fake ? 'fake' : 'real');
                decl.depCount++;
            }).then(function () {
                return _this.addDecls(deps.shouldDeps, function (dep) {
//                    if (decl.deps[dep.key]) return;
                    console.log('KK', key + ' -> ' + dep.key, dep.deps);
                });
            });
        });
    },
    ...
    resolve: function () {
        ...
        while (hasChanges && items.length) {
            console.log('!', items.map(v => v.key), result);
            var i, j, l;
            ...
            for (i = 0, l = items.length; i < l; i++) {
                ...
                if (decl.depCount === 0) {
                    ...
                    for (j = 0; j < l; j++) {
                        var subDecl = items[j],
                            subDeclKey = decl.key;
console.log('aaa', subDecl.key, subDecl.deps);
