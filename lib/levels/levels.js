...
module.exports = inherit({
    ...
    getBlockEntities: function (blockName, modName, modVal) {
        ...
        for (var i = 0, l = blocks.length; i < l; i++) {
            block = blocks[i];
            if (modName) {
                if (block.mods[modName] && block.mods[modName][modVal || true]) {
                    files = files.concat(block.mods[modName][modVal || true].files);
                    dirs = dirs.concat(block.mods[modName][modVal || true].dirs);
                }
            } else {
    ...
    getElemEntities: function (blockName, elemName, modName, modVal) {
        ...
        for (var i = 0, l = elems.length; i < l; i++) {
            elem = elems[i];
            if (modName) {
                if (elem.mods[modName] && elem.mods[modName][modVal || true]) {
                    files = files.concat(elem.mods[modName][modVal || true].files);
                    dirs = dirs.concat(elem.mods[modName][modVal || true].dirs);
                }
            } else {
